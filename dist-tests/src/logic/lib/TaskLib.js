export class TaskLib {
     ...
     */
    verifyTask(taskId, taskDef, skillLib, buildingLib) {
        const errors = [];
        const addError = (message) => { errors.push(`[${taskId}] ${message}`); };
        const getUnifiedSkillInfo = (skillOrSpecName) => {
        ...
        const verifyNameDetails = (details, context) => {
            const unifiedSkillsInThisTask = [];
            const skillAndSpecNamesUsedInTask = new Set();
     ...
     */
    verifyAllTasks(skillLib, buildingLib) {
        let verifiedCount = 0;
        let errorCount = 0;
 ...
 */
export function resolveStepPlaceholderFromLib(intermediateText, taskDetails, chosenOptionIndexForStep) {
    ...
    if (placeholders && placeholders.length > 0) {
        ...
        const optionKey = placeholder.substring(1, placeholder.length - 1);
        const optionsArray = taskDetails[optionKey];
