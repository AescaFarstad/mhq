...
export const behTreeDefinitions = {
    'introSequence': () => new BehTree('introSequence', [
        new ExecNode('startWelcome', (_node, state) => {
            effects.startMinigame(state, { name: 'Welcome' });
        }),
        new AwaitEventNode('awaitWelcomeComplete', 'minigameComplete', (eventDef) => eventDef.params?.minigameType === 'Welcome'),
        new ExecNode('startIngress', (_node, state) => {
            effects.startMinigame(state, { name: 'Ingress' });
        }),
        new AwaitEventNode('awaitIngressComplete', 'minigameComplete', (eventDef) => eventDef.params?.minigameType === 'Ingress'),
    ]),
    'cheatIntro': () => new BehTree('cheatIntro', [
        new AwaitEventNode('awaitWelcomeStart', 'minigameStarted', (eventDef) => eventDef.params?.minigameType === 'Welcome'),
        new WaitNode({ durationMin: 0.1 }),
        new ExecNode('cheatWelcome', (_node, state) => {
            effects.applyWelcomeResults(state, { locationId: "aeiga_reika" });
            state.exitMinigame();
        }),
        new WaitNode({ durationMin: 0.1 }),
        new ExecNode('cheatIngress', (_node, state) => {
