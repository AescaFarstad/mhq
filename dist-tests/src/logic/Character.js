 ...
 */
function calculateXpDeltaForLevel(level) {
    if (level <= 1)
        return 0; // No XP needed to reach level 1
    if (level === 2)
        return C.BASE_LEVEL_XP; // 1000 XP to reach level 2
...
(function (Character) {
    function addCharacter(gameState, characterDefId) {
        ...
        const attributeDefs = gameState.lib.attributes.getAttributeDefinitions();
        ...
        const nextLevelXpFormula = (currentLevel) => {
            return calculateTotalXpForLevel(currentLevel + 1);
        };
        const nextLevelXpDeltaFormula = (currentLevel) => {
    ...
    Character.addCharacter = addCharacter;
    /**
     * Adds an initial skill and its specified specializations to the character.
     */
    function addInitialSkillWithSpecializations(character, skillId, initialSkillData, skillDefinitionFromLib, idPrefix, connections) {
    ...
    Character.addInitialSkillWithSpecializations = addInitialSkillWithSpecializations;
    /**
     * Creates or updates a skill and its proficiency for a character.
     */
    function upsertSkillAndProficiency(character, skillId, level, skillDefinitionFromLib, idPrefix, connections) {
        ...
        if (!avgGovAttrCalcStat_ForSkill) {
            const avgGoverningFormula = (inputs) => {
    ...
    Character.upsertSkillAndProficiency = upsertSkillAndProficiency;
    /**
     * Creates or updates a specialization and its proficiency for a character.
     */
    function upsertSpecializationAndProficiency(character, specId, // Globally unique specialization ID
    level, baseSkillDefinitionFromLib, // Definition of the parent skill
    ...
    Character.upsertSpecializationAndProficiency = upsertSpecializationAndProficiency;
    /**
     ...
     */
    function setOrUpdateSkillAndAllSpecializations(character, skillId, skillDefinitionFromLib, level, connections) {
    ...
    Character.setOrUpdateSkillAndAllSpecializations = setOrUpdateSkillAndAllSpecializations;
    /**
     ...
     */
    function getProficiency(character, skillOrSpecId, gameState // To access lib for definitions
    ) {
    ...
    Character.getProficiency = getProficiency;
})(Character || (Character = {}));
/**
