...
function countObfuscatedCharacters(original: string, obfuscated: string): number {
    let count = 0;
...
function getObfuscatedIndices(original: string, obfuscated: string): Set<number> {
...
function testEdgeCases(): TestResult {
    // Empty string
    const emptyResult = TestAssert.equals(
        obfuscateString('', 1), 
        "", 
        "Empty string should remain empty"
    );
    if (!emptyResult.success) return emptyResult;

    // Single character tests
    const singleCharObfuscated = TestAssert.equals(
        obfuscateString('A', 1), 
        "¿", 
        "Single char should be obfuscated with '¿' at 100%"
    );
    if (!singleCharObfuscated.success) return singleCharObfuscated;

    const singleCharAboveThreshold = TestAssert.equals(
        obfuscateString('A', 0.6), 
        "¿", 
        "Single char should be obfuscated with '¿' above 0.5 threshold"
    );
    if (!singleCharAboveThreshold.success) return singleCharAboveThreshold;

    const singleCharAtThreshold = TestAssert.equals(
        obfuscateString('A', 0.5), 
        "A", 
        "Single char should not be obfuscated at 0.5 threshold"
    );
    if (!singleCharAtThreshold.success) return singleCharAtThreshold;

    const singleCharBelowThreshold = TestAssert.equals(
        obfuscateString('A', 0.4), 
        "A", 
        "Single char should not be obfuscated below threshold"
    );
    if (!singleCharBelowThreshold.success) return singleCharBelowThreshold;

    // Zero obfuscation
    const zeroObfuscation = TestAssert.equals(
        obfuscateString('Hello', 0), 
        'Hello', 
        "0% obfuscation should return original string"
    );
    if (!zeroObfuscation.success) return zeroObfuscation;

    return { success: true };
}

function testProgressiveObfuscation(): TestResult {
    ...
    for (const [percentageStr, expected] of Object.entries(expectedResults)) {
        ...
        const testResult = TestAssert.equals(
            result, 
            expected, 
            `Progressive obfuscation at ${percentage * 100}%`
        );
...
function testStability(): TestResult {
...
function testStatisticalDistribution(): TestResult {
...
function testSpacePreservation(): TestResult {
    ...
    for (let i = 0; i < textWithSpaces.length; i++) {
        if (textWithSpaces[i] === ' ') {
            const spacePreserved = TestAssert.equals(
                result[i], 
                ' ', 
                `Space at position ${i} should be preserved`
            );
            if (!spacePreserved.success) return spacePreserved;
        }
    }
...
function testFirstLastCharacterRules(): TestResult {
    const testText = 'Hello';
    
    // At 100% obfuscation, first char should be ¿ and last should be ?
    const fullObfuscation = obfuscateString(testText, 1.0, 0);
    
    const firstCharTest = TestAssert.equals(
        fullObfuscation[0], 
        '¿', 
        'First character should be ¿ at 100% obfuscation'
    );
    if (!firstCharTest.success) return firstCharTest;

    const lastCharTest = TestAssert.equals(
        fullObfuscation[fullObfuscation.length - 1], 
        '?', 
        'Last character should be ? at 100% obfuscation'
    );
    if (!lastCharTest.success) return lastCharTest;
...
export function createStringObfuscationTests(): TestSuite {
    return {
